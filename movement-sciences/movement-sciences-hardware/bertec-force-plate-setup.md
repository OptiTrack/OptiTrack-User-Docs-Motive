...
Configuring force plate sampling rate from [Devices pane](../../motive-ui-panes/devices-pane.md).This page provides instructions on how to integrate a Bertec force plate system with an OptiTrack motion capture system.
...
Set up an OptiTrack system. Connect the camera system to the same host PC. For more information, refer to the [Quick Start Guide: Getting Started](../../quick-start-guides/quick-start-guide-getting-started.md) page or the [Hardware Setup](../../hardware/) pages.
...
![Click image to enlarge.](<../../.gitbook/assets/image (7) (1) (2).png>)
{% endtab %}
...
![Click image to enlarge.](<../../.gitbook/assets/image (10) (2).png>)

![Zero cable from the amplifier](<../../.gitbook/assets/image (247).png>)
{% endtab %}
{% endtabs %}
...
![](<../../.gitbook/assets/image (227).png>) ![](../../.gitbook/assets/DAQmxInstall.gif)
...
* Link: [http://bertec.com/downloads/](http://bertec.com/downloads/)
...
![Motive force plate representation in Perspective view.](<../../.gitbook/assets/image (221).png>) ![Status Log pane in Motive.](<../../.gitbook/assets/image (268).png>)
...
![Bertec force plate with CS-400 aligned properly.](<../../.gitbook/assets/image (702).png>) ![Calibrated force plate position and orientation. X and Y axis is shown.](<../../.gitbook/assets/image (277).png>)
...
If the force plate dimensions are not automatically configured, you need to enter the dimensions of the force plate in the force plate properties after calibrating its positions. Go to the [Devices pane](../../motive-ui-panes/devices-pane.md) and select the force plate, and its properties will get listed under the [Properties pane](../../motive-ui-panes/properties-pane/). Enter the length and width (in inches) values for the corresponding plates as reported in the specifications.
...
{% hint style="info" %}
**Note:** Zeroed scales of Bertec force plates are saved within their software driver, and each time the driver restarts, these settings are refreshed. This means that the force plate zero setting will be refreshed each time you start Motive, or each time the device is disabled and enabled back again in Motive. Please be aware of this behavior and zero your plates when necessary. In Motive, there is a **Zero On Enable** property setting for Bertec force plates under the [Devices pane](../../motive-ui-panes/devices-pane.md), and enabling this setting will automatically zero your plate each time the device is enabled or when Motive restarts. The **Zero On Enable** setting is enabled by default.
{% endhint %}

![Set the force plate data to zero for more accurate data.](<../../.gitbook/assets/image (234).png>)
...
![Configuring force plate sampling rate from Devices pane.](<../../.gitbook/assets/image (211).png>)
...
2. In the [Devices pane](../../motive-ui-panes/devices-pane.md), select the eSync 2 among the listed devices. This will list out the synchronization settings in the properties pane for the selected eSync 2.
...
6. Next step is to configure the output signal so that the clock signal can be sent to the force plate system. Under the _Outputs_ section, enable the corresponding output port of the eSync 2 which the force plate system is connected to.
...
8. Now that the eSync 2 has been configured, you need to configure the force plate properties in Motive. While the force plate(s) is selected in Motive, access the Properties pane to view the [force plate properties](../../motive-ui-panes/properties-pane/properties-pane-force-plates.md). Here, set the following properties:
9. Triggered Sync → _None_
...
![Example eSync properties for clock sync. Make sure the eSync is selected in the Properties pane.](<../../.gitbook/assets/image (820).png>) ![Example force plate properties for clock sync. Make sure the Force Plate is selected in the Properties pane.](<../../.gitbook/assets/image (766).png>)
...
![Applying Biomech preset settings.](<../../.gitbook/assets/image (786).png>)
